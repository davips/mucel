module World (initialWorld, soma, ts, w1) where
import Config; import Geometry; import Bio
import Struct; import Rnd; import Debug; import View
import Dna
...
initialWorld = build timeToCollision $ combo $ d1 $ walls ++ w3

w1 = zipWith Uni [ ParticleInfo 4 (V (-400) 0) (V 100 0) 30
                , ParticleInfo 5 (V 400 0) (V (-100) 8) 30
                , ParticleInfo 6 (V (-50) 800) (V 0 (-2)) 100
                ] $ newdna 42

w3 = concat [
--circle seed 14 (totalCells + 4) 30 0 0
  --          , circle (seed+1) 62 (totalCells + 18) 20 500 500
            circle (seed+2) 150 (totalCells + 4) 14 (-500) (-700)
                          ]
...
correct' vec ai bi
  ...
  | otherwise        = correct' (V.update vec (V.fromList [(bi, b')])) ai (bi + 1)
    where b' = uninvade (vec V.! ai) (vec V.! bi)
...
walls = [Wall $ ParticleInfo 0 (V w2 w2) (V 0 0) 0
...
unic ido xpos ypos xvel yvel rad = Uni (ParticleInfo ido (V xpos ypos) (V xvel yvel) rad) Nothing
--
...
ts orgs = window update $ build timeToCollision $ combo $! orgs
