module Geometry(Vec(V),mag,sca,dist,stopPoint,isNear,decompoe,degrees,add,r,r2,timeToHit,Ball(Ball),ballId) where
...
data Ball = Ball{ballId::Int, ballP::Vec, ballV::Vec, ballR::Float}
...
-- buildHeap :: [Ball] -> HeapT (Prio MinPolicy A) () -> M.Map (Ball, Ball) Float
-- buildHeap balls = M.fromList $ buildHeap' balls
-- buildHeap' :: [Ball] -> [((Ball, Ball), Float)]
